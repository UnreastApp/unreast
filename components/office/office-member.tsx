...
export const OfficeMember = ({member, office}: OfficeMemberProps) => {
    ...
    const onClick = () => {
        router.push(`/offices/${office.id}/conversation/${member.id}`);
    }

    return(
        <button onClick={onClick} className={cn("group px-2 py-2 text-sm font-medium text-foreground rounded-sm flex items-center gap-x-2 w-full hover:bg-border")}>
